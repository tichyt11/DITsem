    ...
    for i in range(epochs):
        ...
        if (i+1) % n_e_info == 0:  # print stats
            with T.no_grad():
                ...
                loss_for_info = criterion(eval_X, eval_Y)/eval_Y.size(0)  # loss averaged over batches
            print("Epoch {}/{}, average sequence loss: {} , finished after {} minutes".format(i+1, params["epochs"],
                    loss_for_info, int((time()-t_start)//60)))
            if (i+1) % (10*n_e_info) == 0:  # checkpoint every 10 stat infos
                save_model(model, loss_for_info, params, i + 1)
