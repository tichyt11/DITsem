...
class Classifier(nn.Module):
    def __init__(self, in_features=1, h1_size=40, h2_size=60, h3_size=60):
        super(Classifier, self).__init__()
        self.lstmdim = 3
        self.lstm1 = T.nn.LSTM(1, h1_size, self.lstmdim, batch_first=True)
        self.lstm2 = T.nn.LSTM(in_features - 1, h1_size, self.lstmdim, batch_first=True)

        self.lin_11 = T.nn.Linear(h1_size*self.lstmdim, h2_size)
        self.lin_12 = T.nn.Linear(h1_size*self.lstmdim, h2_size)

        self.lin_2 = T.nn.Linear(2*h2_size, h3_size)
        self.lin_3 = T.nn.Linear(h3_size, h3_size)
        self.lin_out = T.nn.Linear(h3_size, 1)

    def forward(self, X):
        # encoder part
        out1, _ = self.lstm1(T.unsqueeze(X[:, :, 0], 2))
        out2, _ = self.lstm2(X[:, :, 1:])

        h11 = T.relu(self.lin_11(out1))
        h12 = T.relu(self.lin_12(out2))
        h1 = T.cat(h11, h12, 1)

        h2 = T.relu(self.lin_2(h1))
        h3 = T.relu(self.lin_3(h2))

        Y = T.sigmoid(self.lin_out(h3))
        return Y
